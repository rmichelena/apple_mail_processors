(*
	Taxi Handler - Procesa Correos de Viajes de Taxi
...
property logFile : "~/Library/Logs/MailProcessors_Taxi.log"
...
using terms from application "Mail"
	on perform mail action with messages theMessages for rule theRule
		...
		set processorPath to appPath & "/Contents/Resources/processors/taxi/processor.py"
		
		do shell script "mkdir -p " & quoted form of emlFolderPOSIX
		
		set ts to do shell script "date '+%Y-%m-%d %H:%M:%S'"
		do shell script "echo '[" & ts & "] Taxi: " & (count of theMessages) & " mensaje(s)' >> " & quoted form of logFilePOSIX
		...
		tell application "Mail"
			repeat with i from 1 to (count of theMessages)
				...
				try
					...
					set emlFileName to "taxi_" & ts2 & "_" & i & ".eml"
